...
按"先取证、后改动"排序，每条给出所属子议题与依据。**各条的执行状态见下一节（二之二）**：`#1`、`#2` 已交付，`#5` 阻塞，其余未动。
...
## 二之二、执行状态（2026-09-17 更新）

上表是第 1 轮给出的建议，本身不改。以下是逐条状态与证据；本仓库交付的分支为 `origin/dev_mlz`。

| # | 状态 | 证据 |
|---|---|---|
| 1 | ✅ 已交付 | `f60d4251 chore(deps): raise @prismshadow/agenthub to ^0.4.15`；changelog `changelog/unreleased/2026-09-17-agenthub-0415.md`（+ `.zh.md`）。产物已重建，7364 自 2026-09-17 19:22 起运行该构建 |
| 2 | ✅ 已交付 | `a932c171 feat(core): stamp each Request's visible prefix on request_begin`；changelog `2026-09-17-request-prefix-fingerprint.md`（+ `.zh.md`）；单测 `packages/core/test/request-fingerprint.test.ts` 4/4。**界面呈现不在本条范围内**：本轮决定不在界面展示该指纹，"呈现"归 `#4` |
| 3 | ⬜ 未动 | — |
| 4 | ⬜ 未动 | — |
| 5 | ⛔ 阻塞 | 前置是网关侧是否支持缓存亲和键，尚未确认，故未动上游仓库 |
| 6 | ⬜ 未动 | — |
| 7 | ⬜ 未动 | 该条要解决的场景在本轮真实发生了：2026-09-17 VS Code Remote-SSH 端口转发断线，表现为 "7364 死掉了"（服务端其实健康）。当天只落到 `~/.penguin/server-7364.sh`（`bundle`/`source` 两种模式、端口与数据根可参数化、`status` 报出当前模式），runbook 仍未写 |
| 8 | ⬜ 未动 | — |

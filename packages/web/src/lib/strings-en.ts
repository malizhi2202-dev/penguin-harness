...
export const en: Strings = {
  ...
  /**
   * The Git page: local repositories found under the Project's Workspaces (the directories its
   * Sessions ran in), plus any directory added on the page itself. Everything is read from the
   * .git on this server's disk — no hosting platform is involved.
   */
  git: {
    pageTitle: "Repositories",
    pageDesc:
      "Local git repositories found under this Project's Workspaces. Repositories, commit logs, changes and branches are read straight from the .git on the server's disk, with no hosting platform involved; fetch, pull and push use the server account's own git credentials.",
    repos: "Repositories",
    refresh: "Rescan",
    addDirectory: "Add directory",
    addDirectoryHint:
      "Any directory on the server; it becomes a scan candidate, and whether it is a repository is the server's call.",
    scanning: "Scanning…",
    noRepos: "No git repositories found",
    noReposHint:
      "None of this Project's Workspaces is a repository yet. Run git init in one, or add a directory by hand.",
    selectRepo: "Pick a repository on the left",
    selectFile: "Pick a file on the left to see its diff",
    scanned: (count: number) => `${count} directories scanned`,
    skipped: (count: number) => `${count} more directories were past this scan's limit`,
    readFailed: (message: string) => `Could not read it: ${message}`,
    notARepo: (path: string) => `${path} is not inside a git repository`,
    clean: "Clean",
    dirty: "Changes",
    detached: "Detached HEAD",
    emptyRepo: "No commits yet",
    ...
    binary: "Binary file",
    patchTruncated: "The patch is too large; only its beginning is shown.",
    noChanges: "The working tree is clean",
    noChangesHint: "Nothing staged, modified or untracked.",
    staged: "Staged",
    unstaged: "Unstaged",
    untracked: "Untracked",
    ...
    commitMessagePlaceholder: "What this change does",
    commit: "Commit",
    commitAmend: "Amend the last commit",
    committed: "Committed",
    noStaged: "Nothing is staged",
    diffEmpty: "This file has no diff to show",
    diffUntracked: "Untracked file (not in the index yet)",
    diffStaged: "Staged change",
    diffUnstaged: "Working tree against the index",
    diffCommit: "Commit contents",
    fetch: "Fetch",
    pull: "Pull",
    push: "Push",
    pushUpstream: "Push and set upstream",
    working: "Working…",
    opOutput: "git output",
    opDone: "Done",
    branches: "Branches",
    ...
    switchBody: (name: string) =>
      `Switch to ${name}? Uncommitted changes stay in the working tree; git refuses the switch when they would be lost.`,
    branchSwitched: (name: string) => `Switched to ${name}`,
    remotes: "Remotes",
    noRemotes: "No remote configured",
    path: "Path",
  },

  /**
   * The Alignment timers panel: a Project's `timers.toml`, the timers it declares, and the
   * last alignment pass each one ran. Project-scoped like the Git panel — it answers for the
   * Project's repositories, not for a conversation.
   */
  timers: {
    title: "Alignment timers",
    desc: "Timers are declared in <projectDir>/timers.toml. When one comes due it aligns this Project's repositories: it commits the work tree when asked, fetches and moves each branch up to its upstream, then checks the documentation against the code and — when asked — hands the report to a Session. The file is hand-editable and the system never writes it back.",
    readOnlyHint: "Members are read-only; only the owner can edit the file or run a timer",
    refresh: "Refresh",
    filePath: "File path",
    noAccess: "Cannot read this Project's timers: you may not be a member of it.",
    noTimers: "No timers yet",
    noTimersHint: "Add a [[timer]] entry to the file below, or start from a minimal example.",
    fileMissing: "This Project has no timer file yet",
    fileMissingHint: "Start from a minimal working example; save it and keep editing.",
    createStarter: "Create from example",
    starterCreated: "Example file written",
    parseErrors: "Entries that failed to parse (skipped; the rest still run)",
    fileError: "The file cannot be parsed (nothing is scheduled)",
    /** Field label of the raw-file editor; its "?" explains what saving does, its hint the TOML's shape. */
    fileLabel: "Timers file",
    fileInfo:
      "The whole file is replaced on save and takes effect immediately. The system never writes this file back, so what is written here is the intent itself. One bad entry costs only that entry; a file that cannot be interpreted at all is not written.",
    fileHint:
      "One [[timer]] per task. `name` (a lowercase id) and `start_at` (an ISO 8601 instant) are required; `period` takes 30m / 12h / 7d and may be omitted for a one-off; `sync` is none | fetch | fast-forward | merge; `commit` is off | auto; `docs` is true | false.",
    editFile: "Edit file",
    ...
    commitLabel: "Commit",
    docsOn: "Docs checked",
    docsOff: "Docs skipped",
    running: "Running…",
    runNow: "Run now",
    dryRun: "Dry run",
    dryRunHint: "A dry run only reports; it changes nothing in the repositories.",
    runFinished: (status: string): string => `Run finished: ${status}`,
    dryRunFinished: (status: string): string => `Dry run finished: ${status}`,
    lastReport: "Last report",
    noReport: "No run recorded yet",
    history: "Run history",
    historyEmpty: "No run history yet",
    historyLoadFailed: "Could not load the run history",
    triggerSchedule: "Scheduled",
    triggerManual: "Manual",
    dryRunBadge: "Dry run",
    actions: "Actions",
    findings: "Findings",
    /** A finding's own line is the server's English text, shown as-is; the kind is a fixed enum, so it is translated. */
    findingKinds: {
      dirty_worktree: "Uncommitted changes in the work tree",
      operation_in_progress: "A git operation is in progress",
      detached: "Detached HEAD",
      no_upstream: "No upstream branch",
      diverged: "Diverged from upstream",
      merge_conflict: "Merge conflict (aborted)",
      sync_failed: "Sync failed",
      code_without_docs: "Code changed, docs did not",
      docs_without_code: "Docs changed, code did not",
      broken_doc_refs: "A doc reference points at nothing",
      commits_not_landed: "Commits have not landed upstream",
      read_failed: "The repository could not be read",
    ...
    countsRepos: (n: number): string => `${n} repos`,
    countsFindings: (n: number): string => `${n} findings`,
    countsAttention: (n: number): string => `${n} need attention`,
    countsMerged: (n: number): string => `${n} merged`,
    countsCommitted: (n: number): string => `${n} committed`,
    countsFailed: (n: number): string => `${n} failed`,
    changed: (code: number, docs: number): string => `code ${code} · docs ${docs}`,
  },
...
- candidate_round_limit: \`5\``,
  ...
  errors: {
    ...
    byCode: {
      ...
      timer_running: "This alignment timer is already running; wait for the pass to finish.",
      timer_not_found: "This alignment timer no longer exists: the file may have changed.",

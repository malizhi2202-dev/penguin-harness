...
export const zh = {
  ...
  git: {
    pageTitle: "代码仓库",
    pageDesc:
      "这个项目的工作区里扫出来的本地 git 仓库。仓库、提交日志、改动和分支都直接读服务端磁盘上的 .git，不经过任何托管平台；抓取、拉取、推送使用服务端账户自己的 git 凭据。",
    repos: "仓库",
    refresh: "重新扫描",
    addDirectory: "添加目录",
    addDirectoryHint: "服务端上的任意目录，选定后作为候选扫描；是不是仓库由服务端判定。",
    scanning: "正在扫描…",
    noRepos: "没有扫到 git 仓库",
    noReposHint: "这个项目的工作区里还没有仓库。在某个工作区里 git init，或手动添加一个目录。",
    selectRepo: "从左侧选一个仓库",
    selectFile: "从左侧选一个文件看差异",
    scanned: (count: number) => `已扫描 ${count} 个目录`,
    skipped: (count: number) => `另有 ${count} 个目录超出本次扫描上限`,
    readFailed: (message: string) => `读取失败：${message}`,
    notARepo: (path: string) => `${path} 不在任何 git 仓库里`,
    clean: "干净",
    dirty: "有改动",
    detached: "游离 HEAD",
    emptyRepo: "还没有任何提交",
    aheadBehind: (ahead: number, behind: number) => `领先 ${ahead} · 落后 ${behind}`,
    noUpstream: "没有上游分支",
    tabLog: "提交日志",
    tabChanges: "变更",
    tabBranches: "分支",
    ...
    commitDetail: "提交详情",
    back: "返回日志",
    filesChanged: (count: number) => `${count} 个文件`,
    binary: "二进制文件",
    patchTruncated: "补丁过大，只显示了前面一部分。",
    noChanges: "工作区干净",
    noChangesHint: "没有已暂存、未暂存或未跟踪的文件。",
    staged: "已暂存",
    unstaged: "未暂存",
    untracked: "未跟踪",
    conflicted: "冲突",
    ...
    stageAll: "全部暂存",
    commitMessage: "提交信息",
    commitMessagePlaceholder: "这次改动做了什么",
    commit: "提交",
    commitAmend: "修补上一次提交",
    committed: "已提交",
    noStaged: "没有已暂存的改动",
    diffEmpty: "这个文件没有可显示的差异",
    diffUntracked: "未跟踪文件（还没有加入索引）",
    diffStaged: "已暂存的改动",
    diffUnstaged: "工作区相对索引的改动",
    diffCommit: "提交内容",
    fetch: "抓取",
    pull: "拉取",
    push: "推送",
    pushUpstream: "推送并设置上游",
    working: "执行中…",
    opOutput: "git 输出",
    opDone: "完成",
    branches: "分支",
    ...
    switchBody: (name: string) =>
      `切换到 ${name}？未提交的改动会留在工作区；git 不允许时会拒绝切换。`,
    branchSwitched: (name: string) => `已切换到 ${name}`,
    remotes: "远程",
    noRemotes: "没有配置远程仓库",
    path: "路径",
  },
  ...
  timers: {
    title: "对齐定时器",
    desc: "定时器在 <projectDir>/timers.toml 里声明，到点后对齐这个项目的仓库：按需先提交工作区，再抓取并把分支推进到上游，然后检查文档与代码是否一致，必要时把报告交给一个 Session。文件可手工编辑，系统永不回写。",
    readOnlyHint: "member 只读；只有 owner 可以编辑文件或手动运行。",
    refresh: "刷新",
    filePath: "文件路径",
    noAccess: "无法读取这个项目的定时器：你可能不是它的成员。",
    noTimers: "还没有定时器",
    noTimersHint: "在下面的文件里加一个 [[timer]] 条目，或从一个最小示例开始。",
    fileMissing: "这个项目还没有定时器文件",
    fileMissingHint: "从一个最小可用的示例开始，保存后即可继续编辑。",
    createStarter: "用示例创建",
    starterCreated: "已写入示例文件",
    parseErrors: "解析失败的条目（已跳过，其余照常运行）",
    fileError: "文件无法解析（未调度任何定时器）",
    /** Field label of the raw-file editor; its "?" explains what saving does, its hint the TOML's shape. */
    fileLabel: "定时器文件",
    fileInfo:
      "整份文件覆盖保存，立即生效。系统永不回写这个文件，所以这里写下的就是意图本身。一个条目写错只影响它自己，其余照常运行；整份文件无法解析时不会写入。",
    fileHint:
      "一个 [[timer]] 一个任务。必填 name（小写 id）与 start_at（ISO 8601 时刻）；period 形如 30m / 12h / 7d，留空为一次性；sync 取 none | fetch | fast-forward | merge；commit 取 off | auto；docs 取 true | false。",
    editFile: "编辑文件",
    cancelEdit: "取消编辑",
    nextRun: "下次",
    lastRun: "上次",
    ...
    runNow: "立即运行",
    dryRun: "试运行",
    dryRunHint: "试运行只报告，不改动任何仓库。",
    runFinished: (status: string): string => `运行结束：${status}`,
    dryRunFinished: (status: string): string => `试运行结束：${status}`,
    lastReport: "最近一次报告",
    noReport: "还没有运行记录",
    history: "运行历史",
    historyEmpty: "暂无运行历史",
    historyLoadFailed: "运行历史加载失败",
    triggerSchedule: "定时",
    triggerManual: "手动",
    dryRunBadge: "试运行",
    actions: "动作",
    ...
    findingKinds: {
      ...
      operation_in_progress: "仓库有未完成的 git 操作",
      detached: "游离 HEAD",
      no_upstream: "没有上游分支",
      diverged: "与上游分叉",
      merge_conflict: "合并冲突（已中止）",
      sync_failed: "同步失败",
      code_without_docs: "代码变了，文档没变",
      docs_without_code: "文档变了，代码没变",
      broken_doc_refs: "文档引用指向不存在的目标",
      commits_not_landed: "有提交尚未落到上游",
      read_failed: "仓库读取失败",
    } as Record<string, string>,
    severityAttention: "待处理",
    severityInfo: "提示",
    statusNames: {
      active: "生效",
      disabled: "停用",
    ...
    countsRepos: (n: number): string => `${n} 个仓库`,
    countsFindings: (n: number): string => `${n} 处发现`,
    countsAttention: (n: number): string => `${n} 处待处理`,
    countsMerged: (n: number): string => `合并 ${n}`,
    countsCommitted: (n: number): string => `提交 ${n}`,
    countsFailed: (n: number): string => `失败 ${n}`,
    changed: (code: number, docs: number): string => `代码 ${code} · 文档 ${docs}`,
  },
...
- candidate_round_limit：\`5\``,
  ...
  errors: {
    ...
    byCode: {
      ...
      timer_running: "该对齐定时器正在运行，请等这一轮结束再试。",
      timer_not_found: "该对齐定时器已不存在：文件可能已被改动。",

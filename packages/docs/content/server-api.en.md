...
### Messaging Bindings (Feishu, Telegram, QQ, WeChat, Tuitui)

A Session can be connected to a messaging bot — Feishu, Telegram, QQ, WeChat and Tuitui are the channels today, each under `/messaging/<channel>`. A Session keeps **at most one saved config per channel** (all of them may sit saved side by side), and **at most one of them is enabled** — the enabled channel holds the live connection. Enabling is what binds the bot account to the Session and disabling releases it, so the same app or bot may sit saved on any number of Sessions and only the enable is exclusive. Inbound messages to the bot start Tasks on the Session as ordinary user input, exactly as if typed into the web composer (no marker; queued as follow-ups while the Session is busy, with one heads-up in the chat per busy stretch), and completed replies are relayed back to the chat, chunked under the channel's text-size limit (Telegram's hard cap is 4096 characters). Feishu listens over the SDK's WebSocket long connection, Telegram long-polls `getUpdates`, QQ holds the platform's WebSocket gateway with the `GROUP_AND_C2C_EVENT` intent, WeChat long-polls `ilink/bot/getupdates`, and Tuitui holds one WebSocket event stream — none of them needs a public callback URL. **One binding serves exactly one conversation**: the first one that ever spoke to it — for a direct chat that is the user, for a group or a teams post that is that group or post — and a message from anywhere else is ignored in silence: no reply, no run, and the remembered conversation does not move. A robot that answers whoever spoke last has no identity of its own: two groups mentioning it at once would share one Session's context, and every reply would land in whichever group spoke most recently. A conversation that was never invited should not learn from a refusal that the robot exists either. The remembered conversation is the binding row's own `lastChatId`, so the rule needs no new field and survives a restart; to move a binding to a different conversation, delete the channel's configuration in the Web App and add it again. A Telegram forum topic counts as a conversation of its own, so a binding locks onto its first topic. Saving and connecting are separate: PUT persists credentials only, and the explicit state endpoint owns the connection. Paths omit the `/api/sessions/:sessionId` prefix like the table above.
...
**WeChat carries direct chats only, and the most media of the five.** The claw bot channel has no group inbound at all: a message addressed to the bot in a group never reaches this API, so a binding that answers a direct chat perfectly is silent in a group by design rather than by misconfiguration. In exchange it is the only channel here that carries text, images and files in BOTH directions — a reply's pictures and attachments are uploaded to the platform's CDN (AES-128-ECB under a per-file key) and arrive as real images and files rather than being refused. Two inbound kinds are folded: a voice message is relayed as WeChat's own transcription of it, and a video arrives as a file; a recording the platform could not transcribe reaches the chat as the shared not-supported notice.

**Tuitui carries its credentials in the URL of every call, which makes a URL a secret.** The platform has no OAuth and mints no token: `appId` and `appSecret` ride along with every call — as `?auth=<appId>.<appSecret>` on the event socket and as `?appid=…&secret=…` on each HTTP request. Nothing is exchanged, nothing expires, nothing is refreshed, so every URL this server builds is a credential and no error may quote one. `host` is the only field here that is not secret, and it is REQUIRED — the deployment decides where the platform lives, so this product assumes nothing; only a bare host name is accepted, and a scheme, a path or an explicit `:port` is refused (the port is the platform's own 8282 and is not configurable), because the URL it is pasted into would either drop them silently or end up with two ports.

**The event stream is one long connection, and every frame is acknowledged.** A single WebSocket carries every conversation the robot takes part in — there is no per-chat subscribe frame and no callback URL to expose. A frame carrying a top-level `event_id` is answered with `{"ack": "<event_id>"}` BEFORE it is read, and the platform redelivers anything unacknowledged, so acknowledgement and deduplication stay two separate things that both hang off `event_id`. A dropped connection reconnects on a 2/5/10/30/60-second backoff and then stays at the last step for good.

**Only a message that addresses the robot is relayed from a group, and an outbound message can neither quote nor be a picture.** The platform pushes EVERY message of every group the robot belongs to — the other four channels deliver only what addresses the bot — so a group message without the platform's own `at_me` flag is dropped in the connector, while a direct chat is never gated; a teams post reads as a group and is gated the same way. Outbound, the platform has no reply-to field, so a reply is a message in the same conversation; a sent text cannot be edited afterwards; and an image goes out as the same `msgtype: "file"` a file does (only the upload distinguishes `type`, the send does not), so pictures arrive as files — a known capability downgrade. A teams post is the one conversation shape that renders Markdown, and it accepts no uploaded attachments. Inbound, voice and video are not delivered as media: they reach the conversation as a line of text carrying their URL.

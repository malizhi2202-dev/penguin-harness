...
**Tuitui marks a message it has taken in with a「收到」emoji receipt.** The gesture lands on the inbound message itself (`emoji_reaction` on `POST /message/custom/modify`) and sits after the redelivery check and before the run: an event the platform repeats is acknowledged once, and a message type the robot cannot read is acknowledged too — a receipt says the robot HAS it, not that the robot will answer it. It is a courtesy rather than a reply, so a platform that refuses one costs the gesture and nothing else: the binding records no delivery failure (`lastDeliveryError` answers "did my reply get out") and the run is unaffected. Tuitui is the only channel that declares the capability today; Feishu, Telegram, QQ and WeChat skip it outright.

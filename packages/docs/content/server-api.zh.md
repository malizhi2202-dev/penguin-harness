...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET / PUT | /timers | 读取该 Project 的定时器文件、运行状态与下次运行 / 整体替换该文件 |
| POST | /timers/:name/run | 立即运行某个定时器；`{dryRun: true}` 只出报告、不改动工作区（已有运行在进行时返回 409） |
| GET | /timers/:name/runs | 该定时器的运行历史，最新在前（`limit`） |

读取对任何成员开放；写入与手动运行仅限 Owner。文件整体替换，并用与手工编辑相同的解析器校验：完全无法解析的文件返回 400 且不落盘；个别条目不可用的文件会被保存，并在 `errors` 中返回该条目的错误。定时器遍历的仓库即该 Project 各 Workspace 所在的仓库——没有仓库列表可配置。
...
### 消息绑定（飞书、Telegram、QQ、微信、推推）

Session 可以接入消息软件机器人——目前的渠道是飞书、Telegram、QQ、微信与推推，各自挂在 `/messaging/<channel>` 之下。一个 Session **每个渠道至多保存一份配置**（多份可同时保存），其中**至多一个渠道处于启用状态**——启用的渠道持有在线连接。启用即把机器人账号绑定到该 Session，停用即解除绑定，因此同一个应用或机器人可以同时保存在任意多个 Session 上，只有启用是互斥的。发给机器人的消息以普通用户输入在该 Session 上发起 Task——与在网页输入框里输入完全一致（无标记块；忙碌时排入 follow-up 队列，并在会话里回一句「已排队」，每段忙碌只回一次）——完成的回复再转发回对应会话，并按渠道文本上限分段（Telegram 硬上限 4096 字符）。飞书经 SDK 的 WebSocket 长连接接收事件，Telegram 用 `getUpdates` 长轮询，QQ 以 `GROUP_AND_C2C_EVENT` intent 保持平台的 WebSocket 网关连接，微信用 `ilink/bot/getupdates` 长轮询，推推则以一条 WebSocket 长连接承载全部事件——五者都无需公网回调地址。**一份绑定只服务一个会话**：就是第一条对它说话的那个——单聊即该用户，群聊与团队帖子即那个群、那个帖子——此后来自其他会话的消息一律静默忽略，既不回复、也不运行、也不挪动已记住的会话。因为一个谁最后说话就听谁的机器人没有自己的身份：两个群同时 @ 它会共用同一条上下文，而每条回复都落到最后说话的那个群里；从未被邀请的会话也不该从一次拒绝里得知机器人存在。已记住的会话就是绑定行上的 `lastChatId`，所以这条规则不需要新字段，也能跨重启保留；想换到另一个会话，在网页端删除该渠道的配置再重新添加即可。Telegram 的论坛话题各自算一个会话，因此绑定会锁定在第一个话题上。保存与连接是两件事：PUT 只保存凭据，连接由独立的 state 接口开关。路径同上表，省略 `/api/sessions/:sessionId` 前缀。
...
**群聊里只转发 @ 了机器人的消息，外发消息既不能引用、也发不出独立的图片消息。** 平台会推来该机器人所在每个群的**每一条**消息——其余四个渠道只会投递针对机器人的内容——因此不带平台自身 `at_me` 标记的群消息在连接器里直接丢弃，单聊永远不设这道闸门；团队帖子读作群聊，同样受 `at_me` 约束。出站方向，平台没有 reply-to 字段，一次回复就是发进同一个会话；已发出的文本无法事后编辑；图片走的是与文件同一个 `msgtype: "file"`（上传时才区分 `type`，发送时不区分），因此图片以文件形式到达——这是已知的能力降级。团队帖子是唯一会渲染 Markdown 的会话形态，也不接受上传的附件。入站方向，语音与视频不作为媒体送达，只以一行带 URL 的文本进入对话。

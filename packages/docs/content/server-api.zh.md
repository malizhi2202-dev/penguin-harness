...
**收到一条消息时，推推会给它贴一个「收到」表情作为回执。** 回执落在入站消息本身（`POST /message/custom/modify` 的 `emoji_reaction`），位置在去重之后、运行开始之前：平台重复投递的事件只回执一次，机器人读不懂的消息类型同样回执——回执说的是「机器人拿到了」，不是「机器人要回答它」。它是礼节而不是回复，因此平台拒绝时只损失这个手势：绑定不会被记为投递失败（`lastDeliveryError` 回答的是「我的回复有没有发出去」），运行时序也不受影响。今天只有推推声明了这项能力，飞书、Telegram、QQ 与微信直接跳过它。
